...
void getlineReverse(vector<string>& lines, string& tempLine, bool newLine) {
    ...
    if (newLine) {
        ...
        lines[lines.size() - 1] =
            str + lines[lines.size() - 1];  // 添加到最后一行
    }
}
 ...
 * @return time_t
 * 时间戳，从1970年1月1日00时00分00秒（格林威治时间）至现在的总秒数
 */
...
pair<vector<string>, streampos> readLinesReverse(ifstream& file,
    ...
    while (pos > 0 && linesFinished < numLines) {
        ...
        if (linesRead < numLines && !tempLine.empty()) {
            bool newLine =
                linesRead == linesFinished;  // 特殊处理：一行可能远大于4096字节
            getlineReverse(lines, tempLine, newLine);
            ++linesRead;
        }
    }
...
vector<IPRecord> getIpList(string LOG_FILE, const int timecount) {
    vector<IPRecord> ipList;  // 数组，格式类似{"192.168.1.1",
    ...
    unordered_map<string, unsigned int>
        ipMap;  // map，key = IP地址；value =
                // vector中序号，类似{key:"192.168.1.1", value:0}

    const int numLines = 1000;
    bool finished = false;
    time_t cutoffTime =
        0;  // 必须设初值，否则 = 11927552（Tue May 19 1970 09:12:32 GMT+0800）
    while (!finished) {
        ...
        for (auto it = lines.rbegin(); it != lines.rend(); ++it) {
            ...
            if (cutoffTime == 0) {
                cutoffTime =
                    opTime -
                    timecount;  // 初始化截至时间（以最后一条日志记录的时间，作为截止时间）
...
vector<IPRecord> getBlackPermenant(string BLACK_TEMP) {
    ...
    const size_t BLACK_TIMES =
        6;  // 7天内超过6次进临时黑名单，改为进永久黑名单（永久封IP）
    ...
    inFile.seekg(0);
    map<string, int>
        ipCountMap;  // map的key=黑名单IP地址；value=文件中出现多少次（行）
...
/**
 * 临时黑名单逻辑：
 * 1）每6分钟（360秒）超过2600次访问记录，会被加入到临时黑名单BLACK_TEMP [IP
 * 时间戳 次数] 2）7天内至少6次被列入黑名单，会被加入到永久黑名单BLACK_TEMP2
 * @param file 待处理文件
 * @param endPos 结束指针位置
 * @param numLines 读取行数
 * @return pair<vector<string>, streampos> 读取结果列表 | 新指针位置
...
int main() {
    ...
    cout << printCurrentTime() << "blacklist check cycle starts"
         << endl;  // 打印日志信息
    vector<IPRecord> blackList =
        getIpList(LOG_FILE, TIME_RANGE);  // 读取最近6分钟所有访问记录
    blackList =
        findRecExceedLimit(blackList, VISIT_TIMES);  // 统计待进入黑名单的列表
    blackList = removePermanent(
        blackList, BLACK_TEMP2);  // 剔除永久黑名单中的记录（无需处理）
    ...
    if (isMinStep(MIN_STEP)) {
        blackList =
            getBlackPermenant(BLACK_TEMP);       // 获取本次进入永久黑名单的记录
